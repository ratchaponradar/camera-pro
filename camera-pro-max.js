...
/**
 * คำนวณทิศทางและอัปเดต UI เข็มทิศ
 */
function handleOrientation(e) {
    let heading = e.webkitCompassHeading || (e.alpha ? 360 - e.alpha : null);
    if (heading === null) return;

    // ปรับแต่งค่าตามการหมุนหน้าจอ (Landscape)
    ...
    // อัปเดต UI ส่วน Radar
    if (ELEMENTS.wheel) ELEMENTS.wheel.style.transform = `rotate(${-finalDeg}deg)`;
    if (ELEMENTS.dirLetter) ELEMENTS.dirLetter.innerText = getCardinalDirection(finalDeg);
    if (ELEMENTS.dirDegree) ELEMENTS.dirDegree.innerText = `${finalDeg}°`;

    // อัปเดต UI ส่วนข้อความด้านล่าง
    ...
    if (ELEMENTS.needle) ELEMENTS.needle.style.transform = `translateX(-50%) rotate(${finalDeg}deg)`;
}

/**
 * ติดตามพิกัดและความสูง
 */
navigator.geolocation.watchPosition(
    (pos) => {
        const altEl = document.getElementById("altitudeValue");
        if (altEl) altEl.textContent = pos.coords.altitude ? pos.coords.altitude.toFixed(1) : "--";
    },
